...
import type { AdminUser, Job, Quote, ServiceRequest, Technician } from './types';
...
interface ServiceState {
  ...
  quotes: Quote[];
  loaded: boolean;
  setAll: (data: Partial<Pick<ServiceState, 'requests' | 'technicians' | 'jobs' | 'users' | 'quotes'>>) => void;
...
export const useServiceStore = create<ServiceState>((set) => ({
  ...
  upsertQuote: (q) => set((s) => ({ quotes: upsert(s.quotes, q) })),
}));
